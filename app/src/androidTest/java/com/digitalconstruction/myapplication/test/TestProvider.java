 ...
 */
package com.digitalconstruction.myapplication.test;
...
import com.digitalconstruction.myapplication.data.WeatherContract.LocationEntry;
import com.digitalconstruction.myapplication.data.WeatherContract.WeatherEntry;
...
public class TestProvider extends AndroidTestCase {
    ...
    public void insertKalamazooData() {
        ...
        mLocationRowId = ContentUris.parseId(locationInsertUri);

        ContentValues kalamazooWeatherValues = createKalamazooWeatherValues(mLocationRowId);
        Uri weatherInsertUri = mContext.getContentResolver()
    ...
    public void testUpdateAndReadWeather() {
        ...
        ContentValues kalamazooAltered = createKalamazooWeatherValues(mLocationRowId);
    ...
    /**
     * You can't remove a column from a row via a ContentResolver.  But you can give it an empty value.
     * You can remove an entire row via a ContentResolver--but that doesn't matter here.
     * You can't remove an entire row from ContentValues.  But you can give a column for a row an empty value.
     * <p/>
     * This test proves that giving an empty value to Humidity (eg removing it) works the same via our ContentResolver and more direct ContentValues manipulation
     */
    public void testRemoveHumidityAndReadWeather() {
        //Populate our weather database with a row
        insertKalamazooData();

        //Use ContentResolver to put empty value into humidity

        //This value is for selection parameter of update below, we know this will match, we could also just update every row since there is only one
        double mKalamazooHumidity = 1.5;
        ContentValues mUpdateValues = new ContentValues();
        mUpdateValues.put(WeatherEntry.COLUMN_HUMIDITY, "");
        mContext.getContentResolver().update(WeatherEntry.CONTENT_URI, mUpdateValues, WeatherEntry.COLUMN_HUMIDITY + " = ?", new String[]{Double.toString(mKalamazooHumidity)});
        ...
        //create ContentValues for Kalamazoo Weather data, provide row # so will be same as ContentProvider cursor data
        ContentValues kalamazooAltered = createKalamazooWeatherValues(mLocationRowId);

        //remove the humidity value from it (this updates every row, but there is only one)
        kalamazooAltered.put(WeatherEntry.COLUMN_HUMIDITY, "");

        //Verify the ContentResolver cursor and ContentValues have the same data after both manipulations
        TestDb.validateCursor(weatherCursor, kalamazooAltered);
